...
namespace ETHotfix
{
	...
	public sealed class EventSystem
	{
		...
		private readonly Dictionary<AttributeType,List<Type>> allTypes = new Dictionary<AttributeType,List<Type>>();
		...
		void InitTypes()
		{
			...
			var hotTypes = ETModel.Game.Hotfix.GetHotfixTypes();

			foreach (Type type in hotTypes)
			{
				object[] attrs = type.GetCustomAttributes(typeof (ObjectSystemAttribute), false);
				if (attrs.Length > 0)
				{
					AddType(AttributeType.ObjectSystem, type);
					continue;
				}

				attrs = type.GetCustomAttributes(typeof (EventAttribute), false);
				if (attrs.Length > 0)
				{
					AddType(AttributeType.Event, type);
					continue;
				}

				attrs = type.GetCustomAttributes(typeof (ConfigAttribute), false);
				if (attrs.Length > 0)
				{
					...
					{
						AddType(AttributeType.Config, type);
						continue;
					}
				}

				attrs = type.GetCustomAttributes(typeof (PreloadAttribute), false);
				if (attrs.Length > 0)
				{
					AddType(AttributeType.Preload, type);
					continue;
				}

				attrs = type.GetCustomAttributes(typeof (MessageAttribute), false);
				if (attrs.Length > 0)
				{
					MessageAttribute messageAttribute = attrs[0] as MessageAttribute;
					if (messageAttribute != null)
					{
						AddType(AttributeType.OpCode, type);
						continue;
					}
				}
			}
		...
		{
			var types = GetTypes(AttributeType.ObjectSystem);
			foreach (Type type in types)
			{
				...
				//Log.Debug("[EventSystem] Init:" + type.Name);
			...
			types = GetTypes(AttributeType.Event);
			foreach (Type type in types)
			{
		...
		void AddType(AttributeType attri, Type type)
		{
			if (!allTypes.ContainsKey(attri))
			{
				allTypes.Add(attri,new List<Type>());
			}
			
			this.allTypes[attri].Add(type);
		}
		...
		public List<Type> GetTypes(AttributeType attribute)
		{
			return this.allTypes[attribute];
		}
        ...
        public void Disable(Entity component)
        {
            ...
            foreach (IDisableSystem idisableSystem in idisableSystems)
            {
                if (idisableSystem == null)
                {
                    continue;
                }
