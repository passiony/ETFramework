...
namespace ETModel
{
	...
	public sealed class TChannel: AChannel
	{
		...
		public TChannel(IPEndPoint ipEndPoint, TService service): base(service, AChannelType.Connect)
		{
		...
		public TChannel(Socket socket, TService service): base(service, AChannelType.Accept)
		{
			int packetSize = service.PacketSizeLength;
			this.packetSizeCache = new byte[packetSize];
			this.memoryStream = service.MemoryStreamManager.GetStream("message", ushort.MaxValue);
		...
		{
			if (this.ChannelType == AChannelType.Accept)
			{
				this.StartRecv();
			}
			else
